...
###create predictions on death flag
```{r}
library(modelr)
```
...
conf_mat <- confusionMatrix(thrones_test_pred$pred, thrones_test_pred$dth_flag)
```
...
rf_classifier <- ranger(dth_flag ~ .,
                        ...
                        num.trees = 1000,
                        mtry = 2,
                        min.node.size = 5) #hyperparameters (model settings)
```
...
```{r}
thrones_test_pred <- thrones_test %>% 
  mutate(prediction = predict(rf_classifier, thrones_test)$prediction)
```
```{r}
confusionMatrix(thrones_test_pred$prediction, thrones_test_pred$dth_flag)
```
